# Versiones & Cangelog

## Ver 1 [(PDF)][1]

**Libretas escritas en papel y escaneadas**, 12 Enero 2023 a 4 Enero 2025

PDF contiene / cambios:

* imágenes escaneadas de todas las libretas (276 paginas de 3 libretas)
* nada mas, eso es todo


	
## Ver 2 [PDF][2]

**Agregado temario al final del documento**, 5 Enero 2025

PDF contiene / cambios:

* imágenes escaneadas de todas las libretas
* temario / indice / TOC al final del PDF con temas disponibles para buscar con CTR+F (win) o CMD+F (mac)
	
	
## Ver 3 [PDF][3]

**Temario con hipervínculos**, 9 Enero 2025 - 16 Noviembre 2025

Cambios:

* Nueva portada
* Todas las paginas fueron limpiadas
* Agregado titulo a cada tema
* Indice / Temario movido a pagina numero 3 (antes 277)
* Agregado hipervínculos en indice (click en tema del indice te lleva a su respective pagina)
* Agregado sección de "Cambios y versiones"
	
[1]: https://github.com/alex-esc/Libreta-fermatta/releases/tag/lib
[2]: https://github.com/alex-esc/Libreta-fermatta/releases/tag/lib2
[3]: https://github.com/alex-esc/Libreta-fermatta/releases/tag/lib3

------------

## Ver 4

**Actualmente en progreso...** 


- Transcribiendo imágenes a texto manualmente: (305 pág totales)
	* Progreso manual   2%, 16 Noviembre 2025
	* Progreso manual   5%, 18 Noviembre 2025
	* Progreso manual 13%, 19 Noviembre 2025
	* Progreso manual 20%, 24 Noviembre 2025
	* Progreso manual 25%, 28 Noviembre 2025
- Transcribiendo imágenes a texto automáticamente: ([mas info][minf])
	* Creación de Script de automatización,  3-4 Abril 2026
	* Test y debugging de Script,                     10 Abril 2026
	* Progreso automatizado  36%,  11 Abril 2026
	* Progreso automatizado 100%, 12 Abril 2026
- Implementación de transcripción a documento: 
	* Progreso manual     24%, 13 Abril 2026
	* Progreso manual     77%, 14 Abril 2026
	* Progreso manual   100%, 15 Abril 2026
- Corrección Manual: 
	* Progreso manual     0%, 16 Abril 2026
	
[minf]: https://github.com/alex-esc/Libreta-fermatta/tree/main/Automatizacion

---------

# Futuras Versiones

* __Ver 5__ Próximamente:
	
	* Ordenar por materias
	* Agregar material adicional / multimedia
	* Agregar Introducción
	* Agregar Temas faltantes de 2020-2022

* __Ver 6__ Próximamente:
	
	* Desarrollo de temas, agregar explicaciones en lugar de solo apuntes
	

